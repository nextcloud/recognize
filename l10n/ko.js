OC.L10N.register(
    ...
    "Background Jobs are not executed via cron. Recognize requires background jobs to be executed via cron." : "백그라운드 작업이 cron을 통해 실행되지 않습니다. Recognize는 cron을 통해 백그라운드 작업이 실행되어야 합니다",
    "The app is installed and will automatically classify files in background processes." : "앱이 설치되었으며 백그라운드 작업에서 파일을 자동으로 분류합니다.",
    "None of the tagging options below are currently selected. The app will currently do nothing." : "현재 선택된 태깅 옵션이 없습니다. 앱은 현재 아무 작업도 수행하지 않습니다.",
    ...
    "Face recognition:" : "얼굴 인식:",
    "Queued files" : "대기 중인 파일",
    "Last classification: " : "마지막 분류:",
    "Scheduled background jobs: " : "예약된 백그라운드 작업:",
    "Last background job execution: " : "마지막 백그라운드 작업 실행:",
    "There are queued files in the face recognition queue but no background job is scheduled to process them." : "얼굴 인식 대기열에 대기 중인 파일이 있지만, 이를 처리할 백그라운드 작업이 없습니다.",
    "Face clustering:" : "얼굴 군집화:",
    "faces left to cluster" : "군집화할 남은 얼굴 수",
    "Last clustering run: " : "마지막 군집화 실행:",
    "A minimum of 120 faces per user is necessary for clustering to kick in" : "사용자당 최소 120개의 얼굴이 필요합니다. 그래야 군집화가 시작됩니다.",
    "Enable face recognition (groups photos by faces that appear in them; UI is in the photos app)" : "얼굴 인식 활성화 (사진에 나타난 얼굴에 따라 사진을 그룹화; 사용자 인터페이스는 Photos 앱에 있음)",
    "The number of files to process per job run (A job will be scheduled every 5 minutes; For normal operation ~500 or more, in WASM mode ~50 is recommended)" : "작업 실행당 처리할 파일 수 (5분마다 작업이 예약됨; 정상적인 작업을 위해 ~500개 또는 그 이상, WASM 모드에서는 ~50개 권장)",
    "Object detection & landmark recognition" : "객체 인식 및 랜드마크 인식",
    ...
    "There are queued files in the object detection queue but no background job is scheduled to process them." : "객체 인식 대기열에 대기 중인 파일이 있지만, 이를 처리할 백그라운드 작업이 없습니다.",
    ...
    "There are queued files in the landmarks queue but no background job is scheduled to process them." : "랜드마크 인식 대기열에 대기 중인 파일이 있지만, 이를 처리할 백그라운드 작업이 없습니다.",
    "Enable object recognition (e.g. food, vehicles, landscapes)" : "객체 인식 활성화 (예: 음식, 차량, 풍경)",
    "The number of files to process per job run (A job will be scheduled every 5 minutes; For normal operation ~100 or more, in WASM mode ~20 is recommended)" : "작업 실행당 처리할 파일 수 (5분마다 작업이 예약됨; 정상적인 작업을 위해 ~100개 또는 그 이상, WASM 모드에서는 ~20개 권장)",
    ...
    "The number of files to process per job run (A job will be scheduled every 5 minutes; For normal operation ~20 or more, in WASM mode ~5 is recommended)" : "작업 실행당 처리할 파일 수 (5분마다 작업이 예약됨; 정상적인 작업을 위해 ~ 20개 또는 그 이상, WASM 모드에서는 ~5개 권장)",
    "Reset" : "초기화",
    "Click the button below to remove all tags from all files that have been classified so far." : "지금까지 분류된 모든 파일에서 모든 태그를 제거하려면 아래 버튼을 클릭하세요.",
    "Reset tags for classified files" : "분류된 파일의 태그 초기화",
    "Click the button below to remove all face detections from all files that have been classified so far." : "지금까지 분류된 모든 파일에서 모든 얼굴 인식을 제거하려면 아래 버튼을 클릭하세요.",
    "Reset faces for classified files" : "분류된 파일의 얼굴 초기화",
    "Click the button below to rescan all files in this instance and add them to the classifier queues." : "이 인스턴스의 모든 파일을 다시 스캔하고 분류기 대기열에 추가하려면 아래 버튼을 클릭하세요.",
    "Rescan all files" : "모든 파일 다시 스캔",
    "Click the button below to clear the classifier queues and clear all background jobs. This is useful when you want to do the initial classification using the terminal command." : "아래 버튼을 클릭하면 분류기 대기열을 비우고 모든 백그라운드 작업을 지웁니다. 이는 터미널 명령을 사용하여 초기 분류를 수행하고자 할 때 유용합니다. ",
    "Clear queues and background jobs" : "대기열 및 백그라운드 작업 초기화",
    "Resource usage" : "리소스 사용량",
    "By default all available CPU cores will be used which may put your system under considerable load. To avoid this, you can limit the amount of CPU Cores used. (Note: In WASM mode, currently only 1 core can be used at all times.)" : "기본적으로 사용 가능한 모든 CPU 코어가 사용되며, 이는 시스템에 상당한 부하를 줄 수 있습니다. 이를 피하기 위해 사용하는 CPU 코어의 수를 제한할 수 있습니다. (참고: 현재 WASM 모드에서는 항상 단 1개의 코어만 사용할 수 있습니다.)",
    "Number of CPU Cores (0 for no limit)" : "사용할 CPU 코어 수 (0을 입력하면 무제한)",
    "By default, recognize will only ever run one classifier process at a time. If you have a lot of resources available and want to run as many processes in parallel as possible, you can turn on concurrency here." : "기본적으로 Recognize는 한 번에 하나의 분류기 프로세스만 실행합니다. 많은 리소스를 가지고 있고 가능한 한 많은 프로세스를 병렬로 실행하고 싶다면, 여기서 동시성을 활성화할 수 있습니다.",
    "Enable unlimited concurrency of classifier processes" : "분류기 프로세스의 무제한 동시성 활성화",
    "Tensorflow WASM mode" : "Tensorflow WASM 모드",
    "Checking CPU" : "CPU 확인중",
    "Could not check whether your machine supports native TensorFlow operation. Make sure your OS has GNU lib C, your CPU supports AVX instructions and you are running on x86. If one of these things is not the case, you will need to run in WASM mode." : "Native TensorFlow 작동을 지원 여부를 확인할 수 없습니다. GNU lib C가 설치된 OS를 사용하고, CPU가 AVX 명령을 지원하며, x86에서 실행 중인지 확인하세요. 이 중 하나라도 해당되지 않는 경우, WASM 모드에서 실행해야 합니다.",
    "Your machine supports native TensorFlow operation, you do not need WASM mode." : "이 컴퓨터는 Native TensorFlow 작동을 지원하므로, WASM 모드가 필요하지 않습니다.",
    "WASM mode was activated automatically, because your machine does not support native TensorFlow operation:" : "기계가 Native TensorFlow 작동을 지원하지 않기 때문에 WASM 모드가 자동으로 활성화되었습니다:",
    "Enable WASM mode" : "WASM 모드 활성화",
    "Recognize uses Tensorflow for running the machine learning models. Not all installations support Tensorflow, either because the CPU does not support AVX instructions, or because the platform is not x86 (ie. on a Raspberry Pi, which is ARM), or because the Operating System that your nextcloud runs on (when using docker, then that is the OS within the docker image) does not come with GNU lib C (for example Alpine Linux, which is also used by Nextcloud AIO). In most cases, even if your installation does not support native Tensorflow operation, you can still run Tensorflow using WebAssembly (WASM) in Node.js. This is somewhat slower but still works." : "Recognize는 기계 학습 모델을 실행하기 위해 Tensorflow를 사용합니다. 모든 설치 환경이 Tensorflow를 지원하는 것은 아닙니다. 이는 CPU가 AVX 명령을 지원하지 않거나, 플랫폼이 x86이 아니거나 (예: ARM인 라즈베리 파이), 실행 중인 Nextcloud의 운영 체제(도커를 사용하는 경우, 도커 이미지 내의 OS)가 GNU lib C를 포함하지 않기 때문일 수 있습니다(예: Nextcloud AIO에 사용되는 Alpine Linux). 대부분의 경우, 설치 환경이 Native TensorFlow 작동을 지원하지 않더라도, Node.js에서 WebAssembly (WASM)를 사용하여 TensorFlow를 실행할 수 있습니다. 이는 다소 느리지만 정상 작동합니다.",
    "Tensorflow GPU mode" : "Tensorflow GPU 모드",
    "Enable GPU mode" : "GPU 모드 활성화",
    "Like most machine learning models, Recognize will run even faster when using a GPU. Setting this up is non-trivial but works well when everything is setup correctly." : "대부분의 기계 학습 모델과 마찬가지로, GPU를 사용하면 Recognize가 더 빨리 실행됩니다. 이를 설정하는 것은 간단하지 않지만 모든 것이 올바르게 설정되면 잘 작동합니다.",
    ...
    "Could not load libtensorflow in Node.js. You can try to manually install libtensorflow or run in WASM mode." : "Node.js에서 libtensorflow를 불러오지 못했습니다. 수동으로 libtensorflow를 설치하거나 WASM 모드에서 실행해 볼 수 있습니다.",
    "Successfully loaded libtensorflow in Node.js, but couldn't load GPU. Make sure CUDA Toolkit and cuDNN are installed and accessible, or turn off GPU mode." : "Node.js에서 libtensorflow는 성공적으로 불러왔지만, GPU를 불러오지 못했습니다. CUDA 툴킷과 cuDNN이 설치되어 있고 접근 가능한지 확인하세요. 또는 GPU 모드를 끄세요.",
    "Libtensorflow was loaded successfully into Node.js." : "Node.js에 libtensorflow가 성공적으로 불러와졌습니다.",
    "Could not load Tensorflow WASM in Node.js. Something is wrong with your setup." : "Node.js에서 Tensorflow WASM을 불러오지 못했습니다. 설정에 문제가 있을 수 있습니다.",
    "Tensorflow WASM was loaded successfully into Node.js." : "Node.js에서 Tensorflow WASM이 성공적으로 불러와졌습니다.",
    "If the shipped Node.js binary doesn't work on your system for some reason you can set the path to a custom node.js binary. Currently supported is Node v14.17 and newer v14 releases." : "기본 제공된 Node.js 바이너리가 어떤 이유로든 시스템에서 작동하지 않으면, 사용자 정의 Node.js 바이너리의 경로를 설정할 수 있습니다. 현재 지원되는 버전은 Node v14.17 및 그 이후의 v14 릴리즈입니다.",
    "For Nextcloud Snap users, you need to adjust this path to point to the snap's \"current\" directory as the pre-configured path will change with each update. For example, set it to \"/var/snap/nextcloud/current/nextcloud/extra-apps/recognize/bin/node\" instead of \"/var/snap/nextcloud/9337974/nextcloud/extra-apps/recognize/bin/node\"" : "Nextcloud Snap 사용자의 경우, 각 업데이트로 예정된 경로가 변경되므로 Snap의 \"current\" 디렉토리를 가리키도록 이 경로를 조정해야 합니다. 예를 들어, 이 경로 대신 \"/var/snap/nextcloud/9337974/nextcloud/extra-apps/recognize/bin/node\" 다음 경로로 설정하세요  \"/var/snap/nextcloud/current/nextcloud/extra-apps/recognize/bin/node\"",
    "Classifier process priority" : "분류기 프로세스 우선순위",
    ...
    "Nice value to set the priority of the Node.js processes. The value can only be from 0 to 19 since the Node.js process runs without superuser privileges. The higher the nice value, the lower the priority of the process." : "Node.js 프로세스의 우선순위를 설정하기 위한 Nice 값. Node.js 프로세스는 슈퍼유저 권한 없이 실행되므로 값은 0에서 19 사이여야 합니다. nice 값이 높을수록 프로세스의 우선순위는 낮아집니다.",
    "Terminal commands" : "터미널 명령어",
    "To download all models preliminary to executing the classification jobs, run the following command on the server terminal." : "분류 작업을 실행하기 전에 모든 모델을 다운로드하려면 서버 터미널에서 다음 명령을 실행하세요.",
    "To trigger a full classification run, run the following command on the server terminal. (The classification will run in multiple background jobs which can run in parallel.)" : "전체 분류 작업을 트리거하려면 서버 터미널에서 다음 명령을 실행하세요. (분류는 병렬로 실행될 수 있는 여러 백그라운드 작업에서 실행됩니다.)",
    ...
    "Before running a full initial classification run on the terminal, you should stop all background processing that Recognize scheduled upon installation to avoid interference." : "터미널에서 전체 초기 분류 작업을 실행하기 전에, Recognize가 설치 시 예약한 모든 백그라운드 처리를 중단해야 합니다. 이는 간섭을 피하기 위함입니다.",
    "To run a face clustering run on for each user in the terminal, run the following. Consider adding the parameter --batch-size <count> for large libraries to avoid PHP memory exhaustion. (The clustering will run in sequence inside your terminal.)" : "터미널에서 각 사용자의 얼굴 군집화 작업을 실행하려면 다음을 실행하세요. 큰 라이브러리의 경우 PHP 메모리 고갈을 방지하기 위해 --batch-size <count> 매개변수를 추가하는 것을 고려하세요. (군집화는 터미널 내에서 순차적으로 실행됩니다.)",
    "To remove all face clusters but keep the raw detected faces run the following on the terminal:" : "모든 얼굴 군집을 제거하고 원래 인식된 얼굴은 유지하려면 터미널에서 다음을 실행하세요:",
    "To remove all detected faces and face clusters run the following on the terminal:" : "모든 인식된 얼굴과 얼굴 군집을 제거하려면 터미널에서 다음을 실행하세요:",
    "You can reset the tags of all files that have been previously classified by Recognize with the following command:" : "Recognize가 이전에 분류한 모든 파일의 태그를 다음 명령으로 초기화할 수 있습니다:",
    "You can delete all tags that no longer have any files associated with them with the following command:" : "더 이상 연결된 파일이 없는 모든 태그를 삭제하려면 다음 명령을 사용하세요:",
    "To remove tags that were created by Recognize version 2 from all files run the following on the terminal:" : "Recognize 버전 2에 의해 생성된 태그를 모든 파일에서 제거하려면 터미널에서 다음을 실행하세요:",
