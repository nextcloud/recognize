OC.L10N.register(
    ...
    "To run a face clustering run on for each user in the terminal, run the following. Consider adding the parameter --batch-size <count> for large libraries to avoid PHP memory exhaustion. (The clustering will run in sequence inside your terminal.)" : "Para executar no terminal unha agrupación de facianas para cada usuario, execute o seguinte. Considere engadir o parámetro --batch-size <count> para fototecas grandes para evitar o esgotamento da memoria PHP. (A agrupación executarase en secuencia dentro do seu terminal.)",
