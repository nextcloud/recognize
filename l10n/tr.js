OC.L10N.register(
    ...
    {
    "Recognize" : "Tanıma",
    "Audio classification" : "Ses sınıflaması",
    "Classify audios into categories." : "Sesleri kategoriler altında sınıflar.",
    "Audios" : "Sesler",
    "Provide audios to classify" : "Sınıflanacak sesleri belirtin",
    "Categories" : "Kategoriler",
    "The classified categories. Each input audio is mapped to a text containing a comma separated list of categories." : "Sınıflanmış kategoriler. Belirtilen her ses, virgül ile ayrılmış bir kategori listesi bulunan bir yazı ile eşlenir.",
    "Image classification" : "Görsel sınıflaması",
    "Classify images into categories." : "Görselleri kategoriler altında sınıflar.",
    "Images" : "Görseller",
    "Provide images to classify" : "Sınıflanacak görselleri belirtin",
    "The classified categories. Each input image is mapped to a text containing a comma separated list of categories." : "Sınıflanmış kategoriler. Belirtilen her görsel, virgül ile ayrılmış bir kategori listesi bulunan bir yazı ile eşlenir.",
    "Image face recognition" : "Görsel yüz tanıması",
    "Recognize faces in images and return embedding vectors for each face." : "Görsellerdeki yüzleri tanır ve her yüz için gömme vektörlerini döndürür.",
    "Provide images to recognize faces in" : "Yüzlerin tanınacağı görselleri belirtin",
    "Faces" : "Yüzler",
    "The detected faces. Each input image is mapped to a text containing JSON-encoded face descriptions ({x,y,width,height,score,vector,angle} ) separated by line breaks." : "Algılanan yüzler. Belirtilen her görsel, satır sonları ile ayrılmış JSON kodlu yüz açıklamalarının ({x,y,width,height,score,vector,angle}) bulunduğu bir yazı ile eşlenir.",
    "Video classification" : "Görüntü sınıflaması",
    "Classify videos into categories." : "Görüntüleri kategoriler altında sınıflar.",
    "Videos" : "Görüntüler",
    "Provide videos to classify" : "Sınıflanacak görüntüleri belirtin",
    "The classified categories. Each input video is mapped to a text containing a comma separated list of categories." : "Sınıflanmış kategoriler. Belirtilen her görüntü, virgül ile ayrılmış bir kategori listesi bulunan bir yazı ile eşlenir.",
    ...
    "The app is installed and will automatically classify files in background processes." : "Uygulama kuruldu ve arka planda dosyaları otomatik olarak sınıflanacak.",
    ...
    "Classifier backend" : "Sınıflayıcı arka ucu",
    "The recognize_backend ExApp is installed; TaskProcessing mode is recommended." : "Recognize_backend ExApp kurulmuş; TaskProcessing kipi önerilir.",
    "TaskProcessing mode is enabled, but no recognize_backend ExApp was detected. Make sure a TaskProcessing provider for the recognize task types is installed and enabled." : "TaskProcessing kipi açık, ancak bir recognize_backend ExApp bulunamadı. Tanınan görev türleri için bir TaskProcessing sağlayıcısının kurulmuş ve etkinleştirilmiş olduğundan emin olun.",
    "Use Nextcloud TaskProcessing for classification" : "Sınıflama için Nextcloud TaskProcessing kullanılsın",
    "When enabled, Recognize hands files off to a Nextcloud TaskProcessing provider (typically the recognize_backend ExApp) instead of running TensorFlow locally. Hardware checks and Node.js / FFmpeg requirements no longer apply in this mode." : "Açıldığında, TensorFlow işleminin yerel olarak yürütülmesi yerine dosyalar bir Nextcloud TaskProcessing sağlayıcısına (genellikle recognize_backend ExApp) gönderilir. Bu kipte, donanım denetimleri ve Node.js / FFmpeg gereksinimleri geçerli değildir.",
    ...
    "Last classification: " : "Son sınıflama:",
    ...
    "The number of files to process per job run (A job will be scheduled every 5 minutes; For normal operation ~100 or more, in WASM mode ~20 is recommended)" : "Görev bir kez yürütüldüğünde işlenecek dosya sayısı. Bir görev 5 dakikada bir çalışır. Normal çalışma için ~100 ya da üzeri, WASM kipinde ~20 önerilir",
    ...
    "The number of files to process per job run (A job will be scheduled every 5 minutes; For normal operation ~20 or more, in WASM mode ~5 is recommended)" : "Görev bir kez yürütüldüğünde işlenecek dosya sayısı. Bir görev 5 dakikada bir çalışır. Normal çalışma için ~20 ya da üzeri, WASM kipinde ~5 önerilir",
    "Reset" : "Sıfırla",
    "Click the button below to remove all tags from all files that have been classified so far." : "Şimdiye kadar sınıflanmış tüm dosyalardaki tüm etiketleri kaldırmak için aşağıdaki düğmeye tıklayın.",
    "Reset tags for classified files" : "Sınıflanmış dosyalardan etiketleri kaldır",
    "Click the button below to remove all face detections from all files that have been classified so far." : "Şimdiye kadar sınıflanmış tüm dosyalardaki tüm yüz tanımalarını kaldırmak için aşağıdaki düğmeye tıklayın.",
    "Reset faces for classified files" : "Sınıflanmış dosyalardan tanınan yüzleri kaldır",
    "Click the button below to rescan all files in this instance and add them to the classifier queues." : "Bu Nextcloud kopyasındaki tüm dosyaları yeniden taramak ve bunları sınıflama sırasına eklemek için aşağıdaki düğmeye tıklayın.",
    "Rescan all files" : "Tüm dosyaları yeniden tara",
    "Click the button below to clear the classifier queues and clear all background jobs. This is useful when you want to do the initial classification using the terminal command." : "Sınıflayıcı kuyruklarını ve tüm arka plan işlerini temizlemek için aşağıdaki düğmeye tıklayın. Bu seçenek, ilk sınıflamayı uç birim komutunu ile yapmak istediğinizde kullanışlıdır.",
    ...
    "By default, recognize will only ever run one classifier process at a time. If you have a lot of resources available and want to run as many processes in parallel as possible, you can turn on concurrency here." : "Tanıma, varsayılan olarak aynı anda yalnızca bir sınıflama işlemi yürütür. Çok fazla kaynağınız varsa ve olabildiğince çok sayıda işlemi paralel olarak yürütmek istiyorsanız eş zamanlı yürütmeyi açabilirsiniz.",
    "Enable unlimited concurrency of classifier processes" : "Sınırsız eş zamanlı sınıflama işlemi yürütülebilsin",
    ...
    "WASM mode was activated automatically, because your machine does not support native TensorFlow operation:" : "Bilgisayarınız yerel TensorFlow işlemini desteklemediğinden WASM kipi otomatik olarak açıldı:",
    ...
    "Could not load libtensorflow in Node.js. You can try to manually install libtensorflow or run in WASM mode." : "Libtensorflow node.js içine yüklenemedi. Libtensorflow uygulamasını el ile kurmayı ya da WASM kipinde çalıştırmayı deneyebilirsiniz.",
    ...
    "Could not execute the FFmpeg binary. You may need to set the path to a working binary manually." : "FFmpeg binary dosyası yürütülemedi. Yürütülebilir bir binary dosyasının yolunu el ile ayarlamanız gerekebilir.",
    ...
    "Classifier process priority" : "Sınıflama işlemi önceliği",
    "Checking Nice binary" : "Nice kitaplığı denetleniyor",
    "Could not find the Nice binary. You may need to set the path to a working binary manually." : "Nice dosyası bulunamadı. Yürütülebilir bir binary dosyasının yolunu el ile ayarlamanız gerekebilir.",
    ...
    "To download all models preliminary to executing the classification jobs, run the following command on the server terminal." : "Sınıflama işlerini yürütmeye yönelik tüm modelleri indirmek için sunucu uç biriminde şu komutu yürütün.",
    "To trigger a full classification run in the background, run the following command on the server terminal. (The classification will run in multiple background jobs which can run in parallel.)" : "Arka planda yürütülecek tam bir sınıflama işlemini tetiklemek için, sunucu uç biriminde şu komutu yürütün. (Sınıflama, paralel olarak yürütülebilen birden çok arka plan işleminde yapılır.)",
    "To run a full classification run on the terminal, run the following. (The classification will run in sequence inside your terminal; doesn't work with task processing mode)" : "Uç birimde tam bir sınıflama işlemi yapmak için şu komutu yürütün. (Sınıflama, uç birimde sırayla yapılır, görev işleme kipinde çalışmaz)",
    "Before running a full initial classification run on the terminal, you should stop all background processing that Recognize scheduled upon installation to avoid interference." : "Uç birimde tam bir ilk sınıflama komutunu yürütmeden önce, karışıklıkları önlemek için kurulum sırasında zamanlanmış tüm arka plan işlemlerini durdurmalısınız.",
    "To run a face clustering run on for each user in the terminal, run the following. Consider adding the parameter --batch-size 10000 for large libraries to avoid PHP memory exhaustion. (The clustering will run in sequence inside your terminal.)" : "Terminaldeki her kullanıcıya yüz tanıma işlemi yapmak için şunları yürütün. PHP belleğinin tükenmesini önlemek amacıyla büyük kitaplıklar için --batch-size 10000 parametresini eklemeyi düşünün. (Tanıma terminalinizde sırayla yapılır.)",
    ...
    "You can reset the tags of all files that have been previously classified by Recognize with the following command:" : "Tanıma uygulaması tarafından daha önce sınıflanmış tüm dosyaların etiketlerini şu komutla sıfırlayabilirsiniz:",
    "You can delete all tags that no longer have any files associated with them with the following command:" : "Daha önce tanınarak sınıflanmış tüm dosyaların etiketlerini şu komutla silebilirsiniz:",
