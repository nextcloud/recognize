OC.L10N.register(
    ...
    "To run a face clustering run on for each user in the terminal, run the following. Consider adding the parameter --batch-size <count> for large libraries to avoid PHP memory exhaustion. (The clustering will run in sequence inside your terminal.)" : "Terminaldeki her kullanıcıya yüz tanıma işlemi yapmak için şunları çalıştırın. PHP belleğinin tükenmesini önlemek amacıyla büyük kitaplıklar için --batch-size 1 parametresini eklemeyi düşünün. (Tanıma terminalinizde sırayla yapılır.)",
