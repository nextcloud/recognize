OC.L10N.register(
    ...
    "Enable face recognition (groups photos by faces that appear in them; UI is in the photos app; minimum of 120 faces per user necessary for clustering to kick in)" : "Habilitar reconocimiento facial (agrupar foros por caras que aparecen en ellas; la IU está en la app de fotos; se necesita un mínimo de 120 caras por usuario para que el agrupamiento inicie)",
    ...
    "Recognize uses Tensorflow for running the machine learning models. Not all installations support Tensorflow, either because the CPU does not support AVX instructions, or because the platform is not x86 (ie. on a Raspberry Pi, which is ARM), or because the Operating System that your nextcloud runs on (when using docker, then that is the OS within the docker image) does not come with GNU lib C (for example Alpine Linux, which is also used by Nextcloud AIO). In most cases, even if you installation does not support native Tensorflow operation, you can still run Tensorflow using WebAssembly (WASM) in Node.js. This is somewhat slower but still works." : "Recognize utiliza TensorFlow para ejecutar los modelos de machine learning. No todas las instalaciones soportan TensorFlow, ya sea por que el CPU no soporta instrucciones AVX, o por que la plataforma no es x86 (p.ej. en un Raspberry Pi, que es ARM), o bien por que el sistema operativo donde su nextcloud se ejecuta (cuando utiliza docker, entonces sería el SO dentro de la imagen docker) no viene con GNU lib C (por ejemplo, en Alpine Linux, que también es utilizado por Nextcloud AIO). En la mayoría de los casos, incluso si su instalación no soporta la operación nativa de TensorFlow, puede ejecutarlo usando WebAssembly (WASM) en Node.js. Esto es un poco más lento pero funciona.",
    "Tensorflow GPU mode" : "Modo GPU de Tensorflow",
    "Enable GPU mode" : "Habilitar modo GPU",
    "Like most machine learning models, recognize will run even faster when using a GPU. Setting this up is non-trivial but works well when everything is setup correctly." : "Cómo la mayoría de los modelos de machine learning, recognize se ejecutará aún más rápido utiizando un GPU. Configurar esto no es trivial pero funciona muy bien cuando todo se configura correctamente.",
    ...
    "Before running a full initial classification run on the terminal, you should stop all background processing that recognize scheduled upon installation to avoid interference." : "Antes de ejecutar una clasificación inicial completa en la terminal, debería detener todo el procesamiento en segundo plano que recognize agendó al ser instalado para evitar interferencias.",
    ...
    "To remove tags that were created by recognize version 2 from all files run the following on the terminal:" : "Para quitar todas las etiquetas que fueron creadas por recognize versión 2 de todos los archivos, ejecute lo siguiente en el terminal:",
