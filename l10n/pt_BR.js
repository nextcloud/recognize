OC.L10N.register(
    ...
    "To run a face clustering run on for each user in the terminal, run the following. Consider adding the parameter --batch-size <count> for large libraries to avoid PHP memory exhaustion. (The clustering will run in sequence inside your terminal.)" : "Para executar um clustering facial para cada usuário no terminal, execute o seguinte. Considere adicionar o parâmetro --batch-size<count> para grandes bibliotecas para evitar o esgotamento da memória do PHP. (O clustering será executado em sequência dentro do seu terminal.)",
