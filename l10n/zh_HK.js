OC.L10N.register(
    ...
    "The systemtags app is enabled." : "已啟用 systemtags 應用程式",
    "The systemtags app is currently disabled. Some features of this app will not work." : "systemtags 應用程式目前已禁用。此應用程式的某些功能將無法使用。",
    ...
    "A minimum of 120 faces per user is necessary for clustering to kick in" : "每個用戶至少需要 120 張臉部照片才能進行人臉分類",
    ...
    "Audio tagging" : "音頻標籤",
    "Audio recognition is working." : "音頻識別中。",
    "An error occurred during audio recognition, please check the Nextcloud logs." : "音頻識別出錯，請查看 Nextcloud 記錄。",
    ...
    "By default, recognize will only ever run one classifier process at a time. If you have a lot of resources available and want to run as many processes in parallel as possible, you can turn on concurrency here." : "默認情況下，recognize 只會同時運行一個分類器進程。如果您有很多可用資源，並且希望盡可能並行運行多個進程，您可以在這裡啟用並發。",
    "Enable unlimited concurrency of classifier processes" : "啟用無限制的分類器進程並行",
    ...
    "If the shipped Node.js binary doesn't work on your system for some reason you can set the path to a custom node.js binary. Currently supported is Node v20.9 and newer v20 releases." : "如果隨附的 Node.js 二進製文件由於某種原因無法在您的系統上運行，您可以將路徑設置為自訂 node.js 二進製文件。目前支持的是 Node v20.9 和更新的 v20 版本。",
