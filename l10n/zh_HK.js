OC.L10N.register(
    ...
    "To run a face clustering run on for each user in the terminal, run the following. Consider adding the parameter --batch-size <count> for large libraries to avoid PHP memory exhaustion. (The clustering will run in sequence inside your terminal.)" : "若要在終端機中為每個用戶運行臉部聚類，請執行以下操作：考慮在大型資料庫中添加參數 --batch-size <count> 以避免 PHP 記憶體耗盡。（聚類將在您的終端機中依序執行。）",
