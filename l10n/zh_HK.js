OC.L10N.register(
    ...
    "Counting queued files" : "點在排隊的檔案的數：",
    ...
    "Reset" : "重設",
    "Click the below button to remove all tags from all files that have been classified so far." : "單擊下面的按鈕可從迄今為止已分類的所有檔案中刪除所有標籤。",
    "Reset tags for classified files" : "重置分類檔案的標籤",
    "Click the below button to rescan all files in this instance and add them to the classifier queues." : "單擊下面的按鈕重新掃描此實例中的所有檔案並將它們添加到分類器隊列中。",
    "Rescan all files" : "重新掃描所有檔案",
    "Terminal commands" : "終端命令",
    "To trigger a full classification run manually, run the following command on the terminal." : "要手動觸發完整分類運行，請在終端上運行以下命令。",
    "To download all models preliminary to executing the classification jobs, run the following command on the terminal." : "要下載執行分類作業之前的所有模型，請在終端上運行以下命令。",
