OC.L10N.register(
    ...
    "Could not execute the FFmpeg binary. You may need to set the path to a working binary manually." : "Nedaří se spustit FFmpeg. Může být třeba ručně nastavit popis umístění fungujícího spustitelného souboru s ním.",
    "FFmpeg {version} binary was installed successfully." : "Spustitelný soubor s FFmpeg {version} byl úspěšně nainstalován.",
    "If the shipped FFmpeg binary doesn't work on your system for some reason you can set the path to a custom FFmpeg binary." : "Pokud společně s aplikací dodávaná binárka FFmpeg na vašem systému z nějakého důvodu nefunguje, je možné nastavit popis umístění uživatelsky určené binárky FFmpeg.",
    ...
    "To run a face clustering run on for each user in the terminal, run the following. Consider adding the parameter --batch-size 10000 for large libraries to avoid PHP memory exhaustion. (The clustering will run in sequence inside your terminal.)" : "Pokud chcete spustit seskupení podle tváře pro každého z uživatelů v terminálu, spusťte následující. V případě rozsáhlých knihoven zvažte přidání parametru --batch-size 10000 , abyste se vyhnuli vyčerpání paměti pro PHP. (Seskupování bude spuštěné postupně ve vašem terminálu.)",
