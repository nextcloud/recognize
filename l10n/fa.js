OC.L10N.register(
    ...
    "To trigger a full classification run, run the following command on the server terminal. (The classification will run in multiple background jobs which can run in parallel.)" : "برای اجرای یک اجرای کامل طبقه‌بندی، دستور زیر را در ترمینال سرور اجرا کنید. (طبقه‌بندی در چندین وظیفه پس‌زمینه اجرا می‌شود که می‌توانند به صورت موازی اجرا شوند.)",
    "To run a full classification run on the terminal, run the following. (The classification will run in sequence inside your terminal.)" : "برای اجرای یک اجرای کامل طبقه‌بندی در ترمینال، موارد زیر را اجرا کنید. (طبقه‌بندی به صورت ترتیبی در داخل ترمینال شما اجرا می‌شود.)",
