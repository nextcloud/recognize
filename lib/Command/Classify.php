...
final class Classify extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		foreach ($this->storageService->getMounts() as $mount) {
			...
			if ($userFilter !== null) {
				$mounts = array_values(array_filter($mounts, fn (ICachedMountInfo $mountInfo) => $mountInfo->getUser()->getUID() === $userFilter));
				if (count($mounts) === 0) {
					// Requested user cannot access this mount — skip the whole storage to avoid needless DB queries.
					continue;
				}
			}
			if (count($mounts) > 0) {
